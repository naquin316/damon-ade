...
export class ChatMastraService {
	...
	/**
	 * Invoke an agent turn programmatically (no UI), reusing the exact path the
	 * interactive `session.sendMessage` tRPC uses. Auth (Claude Max OAuth),
	 * identity (CLAUDE.md), skills (.claude/skills) and MCP (.mastracode/mcp.json)
	 * are all read by mastracode from `cwd` (the agent's folder).
	 *
	 * `onLifecycle` lets the caller (desktop main process) light the tab's dot by
	 * emitting AGENT_LIFECYCLE without this package depending on the app.
	 */
	async invokeAgent(params: {
		...
		prompt: string;
		model?: string;
		onLifecycle?: (phase: "start" | "stop") => void;
	}): Promise<{ sessionId: string }> {
		const runtime = await this.getOrCreateRuntime(params.sessionId, params.cwd);
		runtime.lastErrorMessage = null;
		const userMessage = params.prompt.trim() || "[non-text message]";
		await onUserPromptSubmit(runtime, userMessage);

		// Cheap immediate pre-title so the tab stops showing "New Chat" before the
		// LLM-generated title lands on agent_end. onlyIfEmpty guards against
		// clobbering an existing title; the short retry covers the race with the
		// renderer creating the session record.
		const preTitle = userMessage.replace(/\s+/g, " ").trim().slice(0, 60);
		if (preTitle && preTitle !== "[non-text message]") {
			const setPreTitle = () =>
				this.apiClient.chat.updateTitle
					.mutate({
					...
					.then((r) => r.updated)
					.catch(() => false);
			void setPreTitle().then((updated) => {
				if (!updated) setTimeout(() => void setPreTitle(), 1500);
			});
		}
		...
		if (params.onLifecycle) {
			const onLifecycle = params.onLifecycle;
			onLifecycle("start");
			let signalled = false;
			const unsubscribe = runtime.harness.subscribe((event: unknown) => {
				const type = (event as { type?: string } | null)?.type;
				if (type === "agent_end" && !signalled) {
					signalled = true;
					onLifecycle("stop");
					try {
						unsubscribe?.();
		...
		// Fire-and-forget: returns immediately, the tab polls getDisplayState().
		void runtime.harness.sendMessage({ content: params.prompt });
