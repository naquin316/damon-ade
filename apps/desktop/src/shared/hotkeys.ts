...
export function matchesHotkeyEvent(
	...
	// On Mac, Shift+[ produces { and Shift+] produces }
	// Use event.code to match bracket keys when shift is pressed
	if (key === "[" && eventCode === "bracketleft") return true;
	if (key === "]" && eventCode === "bracketright") return true;
...
export const HOTKEYS = {
	...
	// Workspace - switch with ⌘+Shift+1-9
	...
	CYCLE_TASK_TAB: defineHotkey({
		keys: "ctrl+alt+t",
		label: "Cycle Task Tab",
		category: "Navigation",
		description:
			"Cycle through All/Active/Backlog task filters. Map your Logitech mouse button to this shortcut.",
	}),
	CYCLE_PROJECT_FILTER: defineHotkey({
		keys: "ctrl+alt+1",
		label: "Cycle Project Filter",
		category: "Navigation",
		description: "Cycle through project filters on the tasks page.",
	}),
