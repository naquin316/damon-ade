...
export interface FileViewerState {
	...
	filePath: string;
	/**
	 * Absolute path for files that live OUTSIDE the worktree (e.g. an agent's
	 * memory files under the agent-home dir). When set, content is read by
	 * absolute path via the filesystem router instead of the worktree-relative
	 * changes router. `filePath` is still used for display/name/mode resolution.
	 */
	absolutePath?: string;
