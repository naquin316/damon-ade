...
export const createFilesystemRouter = () => {
	return router({
			...
			.mutation(async ({ input }) => {
				...
				while (true) {
					try {
						...
						finalPath = path.join(notesDir, `${baseName}-${counter}.md`);
						counter++;
					} catch {
						break;
					}
				...
				await fs.writeFile(finalPath, `${header}\n\n${body}\n`, "utf-8");

				// Return path relative to rootPath for file-viewer
				return { relativePath: path.relative(input.rootPath, finalPath) };
			}),
		...
		logSession: publicProcedure
			.input(
				z.object({
					rootPath: z.string(),
					tabName: z.string(),
					action: z.enum(["created", "renamed", "closed"]),
			...
			.mutation(async ({ input }) => {
				...
				// Try to extract session ID from command args first
				const allText = `${input.tabName} ${input.oldName ?? ""}`;
				const resumeMatch = allText.match(/--resume\s+([a-f0-9-]{8,})/i);
				let sessionId = resumeMatch ? resumeMatch[1] : "";

				// If no explicit session ID in command, look up the most recent
				// Claude session from disk (covers `claude --resume` without ID
				// and `claude --dangerously-skip-permissions`)
				if (!sessionId) {
					const isClaude = /\bclaude\b/i.test(allText);
					if (isClaude) {
						try {
							...
							for (const file of jsonlFiles) {
								...
								if (stat.mtimeMs > newest.mtime) {
									newest = { name: file, mtime: stat.mtimeMs };
								}
							}
							if (newest.name) {
							...
							// Claude sessions dir doesn't exist, that's fine
						}
					}
				}
				...
				if (input.action === "created") {
					line = `| ${displayName} | ${sessionId} | ${timestamp} | active | - |`;
				} else if (input.action === "renamed") {
					line = `| ${input.tabName} | ${sessionId} | ${input.createdAt ?? timestamp} | active | Renamed from "${input.oldName}" |`;
				} else if (input.action === "closed") {
					line = `| ~~${displayName}~~ | ${sessionId} | ${input.createdAt ?? timestamp} | ${timestamp} | - |`;
				}
				...
				try {
					...
					existingContent = `# Session Journal\n\nUse \`claude --resume <session-id>\` to resume a session.\n\n| Session | Session ID | Started | Closed | Notes |\n|---------|-----------|---------|--------|-------|\n`;
				}

				await fs.writeFile(logPath, `${existingContent}${line}\n`, "utf-8");
		...
		/**
		 * Read a text file by ABSOLUTE path (used by file-viewer panes for files
		 * that live outside the worktree, e.g. an agent's memory files under the
		 * agent-home dir). Mirrors the shape of changes.readWorkingFile so the
		 * file-viewer content loader can consume either interchangeably.
		 */
		readFileByPath: publicProcedure
			...
			.query(
				...
				> => {
					try {
						...
						if (stats.size > MAX_KEYWORD_FILE_SIZE_BYTES) {
