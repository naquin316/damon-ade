	...
	return router({
		createOrAttach: publicProcedure
			...
			.mutation(async ({ input }) => {
				...
				// First terminal/agent action of the run: request macOS Apple Events
				// access now so the "control other apps" prompt is contextual rather
				// than firing at cold boot. No-op after the first call / off macOS.
				requestAppleEventsAccessOnce();
				const {
					paneId,
					tabId,
					workspaceId,
				...
				try {
					...
					console.log("[Terminal Router] result fields:", {
						paneId,
		...
		setClaudeSessionId: publicProcedure
			.input(
				z.object({
					paneId: z.string(),
