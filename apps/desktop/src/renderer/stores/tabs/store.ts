			...
			}),
			{
				...
				migrate: (persistedState, version) => {
					...
					if (version < 5 && state.panes) {
						for (const pane of Object.values(state.panes)) {
							// biome-ignore lint/suspicious/noExplicitAny: migration from removed chat-mastra pane shape
							const legacyPane = pane as any;
					...
					if (version < 8 && state.panes) {
						// Drop panes for removed pane types ("feed", "chat-mastra") and
						// scrub them out of tab layouts / focused-pane pointers.
						const removedPaneIds = new Set<string>();
						for (const [paneId, pane] of Object.entries(state.panes)) {
							// biome-ignore lint/suspicious/noExplicitAny: migration from removed pane types
							const legacyType = (pane as any).type;
							if (legacyType === "feed" || legacyType === "chat-mastra") {
								removedPaneIds.add(paneId);
								delete state.panes[paneId];
							}
						}
						if (removedPaneIds.size > 0 && Array.isArray(state.tabs)) {
							for (const tab of state.tabs) {
								let newLayout: MosaicNode<string> | null = tab.layout;
								for (const paneId of removedPaneIds) {
							...
							for (const [tabId, paneId] of Object.entries(
								...
								if (removedPaneIds.has(paneId)) {
									delete state.focusedPaneIds[tabId];
								}
							}
						}
					}
