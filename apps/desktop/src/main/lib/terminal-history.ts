...
/**
 * Write a Claude session id into a pane's meta.json without a live
 * HistoryWriter. Read-modify-writes so it doesn't clobber cwd/cols/rows.
 *
 * Used by the cross-Mac sync receive path: the peer's claudeSessionId must be
 * staged into local meta.json BEFORE the pane mounts so the existing
 * staged-resume path resumes the correct conversation.
 */
export async function writeClaudeSessionIdToHistory(
	workspaceId: string,
	paneId: string,
	claudeSessionId: string,
): Promise<void> {
