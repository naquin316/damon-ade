...
export class HistoryManager {
	...
	writeToHistory(
		...
		// Scan for Claude Code session ID in PTY output for auto-resume after restart.
		// Try multiple patterns because Claude Code prints the session UUID in several places:
		//   1. `claude --resume <uuid>` — when user runs resume with explicit UUID
		//   2. `/claude-<uid>/<project>/<uuid>/tasks/` — task output paths Claude prints to terminal
		...
		const taskPathMatch = data.match(
			new RegExp(`/claude-\\d+/[^/]+/(${UUID_RE})/`, "i"),
		);
		const sessionFileMatch = data.match(
