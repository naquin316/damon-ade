...
function reflectHookScript(agentHome: string, _userName: string): string {
	// Terse pointer, not the full protocol. The detailed rules live in
	// .writeback-protocol.md (which this references); echoing them in the Stop
	// hook's `reason` just spams the transcript on every run.
	const reason =
		`[reflect] If anything durable came up this session, update your memory and ` +
		`skills per your write-back protocol ` +
		`(${agentHome}/memory/.writeback-protocol.md), then finish. If not, stop.`;
...
// Skip reflection on trivial sessions (a lone user turn with no tool work) so a
// quick one-shot command doesn't force a reflection turn every single time. Tool
// results are recorded as user-role turns, so any session that used tools or ran
// multiple exchanges still has >1 user turn and reflects exactly as before.
try {
	const tp = data && data.transcript_path;
	if (tp) {
		const transcript = readFileSync(tp, "utf8");
		const userTurns = (transcript.match(/"type":"user"/g) || []).length;
		if (userTurns <= 1) process.exit(0);
	}
} catch {}
